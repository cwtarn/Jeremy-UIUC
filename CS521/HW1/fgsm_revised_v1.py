...
# The network N classifies x as belonging to class 2
...
# ------------------------------------------------------------------------------
# adv_x = TODO
# -> Python raised: "NameError: name 'TODO' is not defined"
# -> The targeted perturbation formula was missing.
#
# CORRECTION PERFORMED:
# ------------------------------------------------------------------------------
# Replaced placeholder with the targeted FGSM step:
#
#     adv_x = x - eps * x.grad.sign()
#
# 1. Used `x.grad` to access the loss gradient w.r.t input: dL(N(x), t) / dx.
# 2. Applied `.sign()` to determine the direction of steepest descent.
# 3. SUBTRACTED (-) the perturbation instead of adding because this is a
#    TARGETED attack: we want to MINIMIZE loss toward target class t = 0.
# 4. Scaled by `eps` (0.5 - 1e-7) so the shift strictly satisfies ||x - adv_x||_inf <= epsReal.
# ==============================================================================
adv_x = x - eps * x.grad.sign()
